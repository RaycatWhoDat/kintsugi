...
describe('error and try', () => {
  test('try success result', () => {
    ...
    expect(ev.evalString('result/value')).toEqual({ type: 'integer!', value: 15 });
    expect(ev.evalString('result/kind')).toEqual({ type: 'none!' });
    expect(ev.evalString('result/message')).toEqual({ type: 'none!' });
    expect(ev.evalString('result/data')).toEqual({ type: 'none!' });
  });
  ...
  test('try catches division by zero', () => {
    ...
    expect(ev.evalString('result/value')).toEqual({ type: 'none!' });
  });
  ...
  test('try catches error with kind + message', () => {
    ...
    expect(ev.evalString('result/ok')).toEqual({ type: 'logic!', value: false });
    expect(ev.evalString('result/kind')).toEqual({ type: 'lit-word!', name: 'test-error' });
    expect(ev.evalString('result/message')).toEqual({ type: 'string!', value: 'oops' });
  });
  ...
  test('try catches error with data', () => {
    ...
    expect(ev.evalString('result/ok')).toEqual({ type: 'logic!', value: false });
    const data = ev.evalString('result/data');
  ...
  test('error with kind only', () => {
    ...
    expect(ev.evalString('result/kind')).toEqual({ type: 'lit-word!', name: 'fail' });
    expect(ev.evalString('result/message')).toEqual({ type: 'none!' });
  });
});
...
describe('try/handle', () => {
  test('handler receives error info and returns value', () => {
    ...
    expect(ev.evalString('result/ok')).toEqual({ type: 'logic!', value: false });
    expect(ev.evalString('result/value')).toEqual({ type: 'integer!', value: 42 });
  });
  ...
  test('inline handler function', () => {
    ...
    expect(ev.evalString('result/value')).toEqual({ type: 'string!', value: 'boom' });
  });
});
